//+------------------------------------------------------------------+
//|                                          GoldSilverScalper_M5.mq5 |
//|                                         M5 EMA Trend Scalper      |
//|                                  Gold (XAUUSD) & Silver (XAGUSD)  |
//+------------------------------------------------------------------+
#property copyright "Vincent Trading Systems"
#property link      ""
#property version   "1.10"
#property description "M5 Optimized - Increased trade frequency version"
#property strict
...
input double   InpRiskPercent        = 0.20;    // Risk per trade (%) - slightly reduced for more trades
input double   InpMaxDailyDrawdown   = 3.0;     // Max daily drawdown (%)
input int      InpMaxTotalPositions  = 6;       // Max total positions (both symbols) - increased
input int      InpMaxPositionsSymbol = 4;       // Max positions per symbol - increased

input group "=== Trade Parameters ==="
input double   InpRiskRewardRatio    = 1.5;     // Risk:Reward ratio (1:X)
input double   InpATRMultiplierSL    = 1.3;     // ATR multiplier for Stop Loss - tighter for M5
input int      InpATRPeriod          = 14;      // ATR period
input int      InpMinATRPeriod       = 20;      // Period for minimum ATR filter
input double   InpMinATRMultiplier   = 0.6;     // Min ATR threshold - relaxed for more signals
...
input int      InpNoEntryBeforeEnd   = 15;      // No new entries X min before session end - reduced for M5

input group "=== News Filter ==="
input bool     InpUseNewsFilter      = true;    // Enable news filter
input int      InpNewsMinutesBefore  = 10;      // Minutes before news to avoid - reduced for M5
input int      InpNewsMinutesAfter   = 10;      // Minutes after news to avoid - reduced for M5
input string   InpNewsCountries      = "USD";   // News countries to monitor (comma separated)

input group "=== Exit Management ==="
input double   InpBreakevenTrigger   = 0.8;     // Move to BE at X times risk (R) - faster for M5
input double   InpTrailingDistance   = 0.8;     // Trailing stop distance (in R) - tighter for M5
...
input double   InpPinBarWickRatio    = 1.8;     // Pin bar wick to body ratio - relaxed for more signals
input bool     InpUseEngulfing       = true;    // Use engulfing pattern entries
input double   InpEngulfingMinSize   = 0.4;     // Min engulfing size (X * ATR) - relaxed for more signals

input group "=== General ==="
input int      InpMagicNumber        = 20240505;// Magic number - unique for M5
input string   InpTradeComment       = "GS_M5"; // Trade comment
...
int OnInit()
{
   ...
   {
      Print("Warning: This EA is designed for M5 timeframe. Current: ", EnumToString(_Period));
   }
   ...
   // Create indicator handles - ALL using PERIOD_M5
   handleEMAFast = iMA(_Symbol, PERIOD_M5, InpEMAFast, 0, MODE_EMA, PRICE_CLOSE);
   handleEMAMedium = iMA(_Symbol, PERIOD_M5, InpEMAMedium, 0, MODE_EMA, PRICE_CLOSE);
   ...
   Print("=================================================");
   Print("GoldSilverScalper M5 (High Frequency) initialized");
   Print("=================================================");
   Print("Symbol: ", _Symbol, " | Timeframe: M5");
   Print("Risk: ", InpRiskPercent, "% | R:R 1:", InpRiskRewardRatio);
   Print("EMA Periods: ", InpEMAFast, "/", InpEMAMedium, "/", InpEMASlow);
...
void OnDeinit(const int reason)
{
   ...
   Print("GoldSilverScalper M5 deinitialized. Reason: ", reason);
}
...
void OnTick()
{
   ...
   // Check if new bar formed - M5 timeframe
...
{
   double open1 = iOpen(_Symbol, PERIOD_M5, shift);
   double close1 = iClose(_Symbol, PERIOD_M5, shift);
   double open2 = iOpen(_Symbol, PERIOD_M5, shift + 1);
   double close2 = iClose(_Symbol, PERIOD_M5, shift + 1);
   double low1 = iLow(_Symbol, PERIOD_M5, shift);
...
bool IsBearishEngulfing(int shift, double emaFast, double emaMedium)
{
   ...
   double open2 = iOpen(_Symbol, PERIOD_M5, shift + 1);
   double close2 = iClose(_Symbol, PERIOD_M5, shift + 1);
   double high1 = iHigh(_Symbol, PERIOD_M5, shift);
