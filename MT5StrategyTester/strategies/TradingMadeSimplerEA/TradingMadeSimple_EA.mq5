...
input double   InpRiskPercent       = 2.0;       // Risk % per Trade (if Lot=0)
...
int OnInit()
{
   ...
   hHMA = iCustom(_Symbol, PERIOD_CURRENT, "TMS_Indicators\\HMA_TMS", 
                  InpHMA_Period, InpHMA_Method, InpHMA_Price);
   if(hHMA == INVALID_HANDLE)
   {
      // Try alternative - built-in implementation
      Print("HMA custom indicator not found, using built-in calculation");
   ...
   hHeikenAshi = iCustom(_Symbol, PERIOD_CURRENT, "Examples\\Heiken_Ashi");
...
void CalculateHMA(int bars)
{
   ...
   double wma1[], wma2[], rawHMA[];
   ArrayResize(wma1, bars + period);
   ArrayResize(wma2, bars + period);
   ArrayResize(rawHMA, bars + period);
   
   // Calculate WMA(period/2)
   for(int i = 0; i < bars + period; i++)
   {
      wma1[i] = iMA(_Symbol, PERIOD_CURRENT, period/2, 0, InpHMA_Method, InpHMA_Price);
      wma2[i] = iMA(_Symbol, PERIOD_CURRENT, period, 0, InpHMA_Method, InpHMA_Price);
   }
   
   // Simple HMA approximation using MAs
   int hWMA1 = iMA(_Symbol, PERIOD_CURRENT, period/2, 0, InpHMA_Method, InpHMA_Price);
   int hWMA2 = iMA(_Symbol, PERIOD_CURRENT, period, 0, InpHMA_Method, InpHMA_Price);
   
   double tempWMA1[], tempWMA2[];
   ArrayResize(tempWMA1, bars);
   ArrayResize(tempWMA2, bars);
   ArraySetAsSeries(tempWMA1, true);
   ArraySetAsSeries(tempWMA2, true);
   
   CopyBuffer(hWMA1, 0, 0, bars, tempWMA1);
   CopyBuffer(hWMA2, 0, 0, bars, tempWMA2);
   
   for(int i = 0; i < bars; i++)
   {
      bufHMA_Main[i] = 2 * tempWMA1[i] - tempWMA2[i];
   ...
   IndicatorRelease(hWMA1);
   IndicatorRelease(hWMA2);
}
