...
input int      TradeWindowEndHour = 11;       // Stop placing new orders after (Hour)
input int      HardExitHour       = 16;       // Close all positions by (Hour)
input int      HardExitMinute     = 0;        // Hard Exit Minute

input group "=== Range Filters ==="
input int      MinRangePips       = 15;       // Minimum Asian Range (Pips) - Skip if smaller
input int      MaxRangePips       = 50;       // Maximum Asian Range (Pips) - Skip if larger
input double   BufferPips         = 5.0;      // Entry Buffer above/below range (Pips)

input group "=== Prop Firm Risk Management ==="
input double   RiskPercent        = 0.5;      // Risk Per Trade (%) - Conservative for prop
input double   MaxDailyDD         = 4.0;      // Max Daily Drawdown (%) - Stop before 5% limit
input double   MaxTotalDD         = 8.0;      // Max Total Drawdown (%) - Stop before 10% limit
input int      MaxTradesPerDay    = 2;        // Maximum trades per day

input group "=== Take Profit Settings ==="
input bool     UseScaledExit      = true;     // Use scaled exit (50% at TP1, trail rest)
input int      TP1_Pips           = 30;       // First Take Profit (Pips) - Close 50%
input double   TP2_RiskMultiple   = 2.0;      // Final TP as multiple of risk (R:R)
input bool     UseTrailingStop    = true;     // Trail stop after TP1
...
input bool     TradeFriday        = false;    // Trade on Friday (usually worse)
...
int OnInit()
{
   ...
   Print("==========================================================");
   Print("London Breakout EA - Prop Firm Edition v1.0");
   Print("==========================================================");
...
void PlaceBreakoutOrders()
{
   ...
   // Calculate TP levels
   double buyTP1 = NormalizeDouble(buyEntry + TP1_Pips * g_pipValue, g_digits);
   double sellTP1 = NormalizeDouble(sellEntry - TP1_Pips * g_pipValue, g_digits);
   double buyTP2 = NormalizeDouble(buyEntry + (buyEntry - buySL) * TP2_RiskMultiple, g_digits);
   double sellTP2 = NormalizeDouble(sellEntry - (sellSL - sellEntry) * TP2_RiskMultiple, g_digits);
   
   // Use TP1 for initial order if scaled exit, otherwise TP2
   double buyTP = UseScaledExit ? buyTP1 : buyTP2;
   double sellTP = UseScaledExit ? sellTP1 : sellTP2;
   ...
   // Place Buy Stop
   if(trade.BuyStop(buyLots, buyEntry, _Symbol, buySL, buyTP, ORDER_TIME_SPECIFIED, expiration, TradeComment + "_BUY"))
   {
   ...
   // Place Sell Stop
   if(trade.SellStop(sellLots, sellEntry, _Symbol, sellSL, sellTP, ORDER_TIME_SPECIFIED, expiration, TradeComment + "_SELL"))
   {
...
void ManageOpenPosition()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      //--- Scaled Exit: Take partial profit at TP1
      if(UseScaledExit && !g_partialTPTaken)
      {
         bool hitTP1 = false;
         double tp1Level = 0;
         ...
         {
            tp1Level = openPrice + TP1_Pips * g_pipValue;
            hitTP1 = (bid >= tp1Level);
         }
         else
         {
         ...
         if(hitTP1)
         {
            ...
            if(closeVolume < currentVolume)
            {
               ...
               {
                  Print("Partial TP taken: ", closeVolume, " lots at TP1 (", TP1_Pips, " pips)");
                  ...
                  double newTP = (posType == POSITION_TYPE_BUY) ?
                                 openPrice + riskDistance * TP2_RiskMultiple :
                                 openPrice - riskDistance * TP2_RiskMultiple;
                  
                  trade.PositionModify(ticket, NormalizeDouble(newSL, g_digits), NormalizeDouble(newTP, g_digits));
                  Print("Remaining position: SL moved to BE, TP set to ", DoubleToString(newTP, g_digits));
               }
            }
         }
      }
      
      //--- Trailing Stop (after partial TP taken or if not using scaled exit)
      if(UseTrailingStop && (g_partialTPTaken || !UseScaledExit))
      {
         ...
         if(posType == POSITION_TYPE_BUY)
         {
            ...
            if(newSL > currentSL + g_pipValue && newSL > openPrice)
            {
...
void DisplayStatus(string phase = "")
{
   ...
   // Build status string
   string status = "";
   status += "═══════ LONDON BREAKOUT - PROP EDITION ═══════\n";
   status += "Phase: " + phase + "\n";
   status += "────────────────────────────────────────────\n";
